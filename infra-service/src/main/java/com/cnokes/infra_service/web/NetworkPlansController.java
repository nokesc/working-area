...
public class NetworkPlansController {
	...
	@Autowired
	MongoNetworkPlanRepo mongoContractRepo;
	...
	public NetworkPlan getByName(@Parameter(example = "name_1") @PathVariable(value = "name") String name) {
		...
		Optional<NetworkPlan> entity = mongoContractRepo.findByName(name);
		if (entity.isPresent()) {
			return entity.get();
		}
	...
	public NetworkPlan get(@Parameter(example = "6186c224c39e9e7f645992d6") @PathVariable(value = "id") String id) {
		LOGGER.info("id=" + id);
		Optional<NetworkPlan> entity = mongoContractRepo.findById(new ObjectId(id));
		if (entity.isPresent()) {
			return entity.get();
		}
		throw new HttpNotFoundException();
	...
	// @ApiResponses(@ApiResponse(responseCode = "404", content = @Content(schema = @Schema(implementation = BaseErrorResponse.class))))
	// @GetMapping(path = "findByNetworksId/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
	// public NetworkPlan getByNetworksId(
	// 		@Parameter(example = "6174322fbf00a07314323d13") @PathVariable(value = "id") String id) {
	// 	LOGGER.info("id=" + id);
	// 	List<NetworkPlan> entity = mongoContractRepo.findByNetworksId(new ObjectId(id));
	// 	if (entity.isEmpty()) {
	// 		throw new HttpNotFoundException();
	// 	}
	// 	return entity.get(0);
	...
	public List<NetworkPlan> getAll() {
		return mongoContractRepo.findAll();
	}

	@PutMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
	public void put(@RequestBody NetworkPlan networkPlan) {
		NetworkPlan save = mongoContractRepo.save(networkPlan);
