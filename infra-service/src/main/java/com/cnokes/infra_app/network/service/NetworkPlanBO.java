...
import com.cnokes.framework.model.IndexedField;
...
public class NetworkPlanBO {
    ...
    private final IndexedField<Network, CIDRBlock, Long> subnetsIndex;

    public NetworkPlanBO(NetworkPlan in) {
        this.networkPlan = JacksonUtils.JSON_DEFAULT_MAPPER.clone(in);
        ...
        if (network == null) {
            ...
            if (cidrBlock == null) {
                throw new HttpBadRequestException(this.networkPlan.getNetwork().getCidrBlock(), Arrays.asList(new FieldError("network.cidrBlock", "IL")));
            }
        }

        subnetsIndex = new IndexedField<Network, CIDRBlock, Long>(
            ...
            @Override
            protected void preInsert_ready(IndexedField<Network, CIDRBlock, Long>.TargetAndField toInsert,
                    int insertLocation) {
                CIDRBlock toInsertCidrBlock = toInsert.getField();
